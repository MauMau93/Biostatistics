...
iii) ii) Confidence Interval between means: high vs none
...
```{r}
#experiment <- data.frame(group=Density, ASO=Mean_Seagrass)
library(pairwiseCI)
pairwiseCI(Mean_Seagrass ~ Density, data = x, 
           method = "Param.diff", conf.level = 0.983, var.equal = TRUE)
```
